			...
			uploadImage: function(file) {
				...
				return new Promise((resolve, reject) => {
					...
					policy().then(response => {
						...
						//上传进度
						me.uploadTask.onProgressUpdate((progressRes) => {
							me.imageList[me.imageList.length - 1].progress = progressRes
								.progress;
						});
					}).catch(err => {
						reject(err.message);
